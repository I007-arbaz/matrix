...
let Main = () => {
  ...
  let getData = async () => {
    let api = await fetch(`https://api.dexscreener.com/latest/dex/search?q=${search}`);
    let res = await api.json();
    setToken(res?.pairs)
  }
  console.log(token);
    ...
    <>
      <div className="main">
        ...
        <div className="middle-body">
          <h1 className="middle-body-header">Token Search Results</h1>
          <div className="middle-body-content">
            {token.splice(0, 10).map((each, index)=> {
                return (
                    <Token data={each} key={index}/>
                )
            })}
          </div>
        </div>
